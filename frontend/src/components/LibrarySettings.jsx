...
function LibrarySettings({ onLibraryUpdate }) {
    ...
    const handleAddPath = () => {
        if (newPath) { fetch(`${API_URL}/api/library/paths`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ path: newPath }) }).then(res => res.json()).then(addedPath => { setPaths([...paths, addedPath]); setNewPath(''); }); }
    };

    const handleDeletePath = (idToDelete) => {
        fetch(`${API_URL}/api/library/paths/${idToDelete}`, { method: 'DELETE', }).then(() => { setPaths(paths.filter(p => p.id !== idToDelete)); });
    };
    ...
    const handleAddYouTubeUrl = () => {
        if (youtubeUrl) {
            ...
            .then(res => {
                if (!res.ok) {
                    // Get the specific error message from the server's response
            ...
            .then(data => {
                setYoutubeUrl('');
                alert(`Successfully added ${data.count} item(s) from the YouTube ${data.type}.`);
                if (onLibraryUpdate) { onLibraryUpdate(); }
            })
            .catch(err => {
                // Display the specific error message in the alert
                console.error("Failed to add YouTube URL:", err);
    ...
    const handleScanLibrary = () => {
      setIsScanning(true);
      fetch(`${API_URL}/api/library/scan`, { method: 'POST' }).then(res => res.json()).then(data => { alert(data.message); setIsScanning(false); if (onLibraryUpdate) { onLibraryUpdate(); } }).catch(error => { alert('Error: Could not start library scan.'); setIsScanning(false); });
    };

    const handlePurgeLibrary = () => {
        if (window.confirm('DANGER: Are you sure you want to purge the entire media library?\n\nThis will remove all media records from the database but will NOT delete your actual files. This action cannot be undone.')) {
            setIsPurging(true);
            fetch(`${API_URL}/api/library/purge`, { method: 'DELETE' }).then(res => res.json()).then(data => { alert(data.message); setIsPurging(false); if (onLibraryUpdate) { onLibraryUpdate(); } }).catch(error => { alert('Error: Could not purge the library.'); setIsPurging(false); });
        }
    };

    const handleShutdownServer = () => {
        if (window.confirm('WARNING: Are you sure you want to shut down BoomServer?\n\nThis will stop the backend server and redirect your browser.')) {
            setIsShuttingDown(true);
            fetch(`${API_URL}/api/shutdown`, { method: 'POST' }).then(res => res.json()).then(data => { alert(data.message); window.location.href = 'https://wapaccess.org'; }).catch(error => { alert('Error: Could not shut down the server. It might already be stopped.'); setIsShuttingDown(false); });
        }
    };
